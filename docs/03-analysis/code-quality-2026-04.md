...
| 파일/영역 | 점수 | 비고 |
|---------|------|------|
| `05-infra/k3s-wsl2/scripts/install-k3s.sh` | 82/100 | 양호. 보안 관련 개선 필요 |
| `05-infra/k3s-wsl2/cluster-setup-recipe.md` (코드블록) | 78/100 | kubeconfig 권한, 테스트 시크릿 이슈 |
| `05-infra/container-security-baseline.md` (코드블록) | 85/100 | 양호. 체계적인 CIS 매핑 |
| `05-infra/policy-as-code/kyverno-policies.md` (YAML) | 88/100 | 양호. 플레이스홀더 키 주의 |
| `05-infra/policy-as-code/opa-gatekeeper.md` (Rego) | 90/100 | 우수. Rego 로직 정확 |
...
| `07-cc-harness/harness-verification-guide.md` (Bash) | 85/100 | 양호. 검증 스크립트 실용적 |
...
| L-04 | `harness-verification-guide.md` | 전체 | 검증 스크립트가 `.md` 내 코드블록으로만 존재 -- 별도 `.sh` 파일로 추출하면 자동화 용이 | `07-cc-harness/scripts/harness-verify.sh`로 분리 권고 |
...
**파일**: `docs/framework/05-infra/k3s-wsl2/scripts/install-k3s.sh` 115행
**파일**: `docs/framework/05-infra/k3s-wsl2/cluster-setup-recipe.md` 114행
...
8. **L-04 검증 스크립트 분리** -- `07-cc-harness/` 내 Bash 검증 스크립트를 별도 `.sh` 파일로 추출하여 CI/CD 자동화 연계
