...
>
> 본 문서의 데이터 등급 분류는 **일반적인 공공기관 SaaS 기준**입니다. 실제 서비스 적용 시:
> - **하단 "상세 분류 기준표"**: 귀사 서비스에서 실제 처리하는 데이터 유형 추가
>   - 예: 민원 처리 데이터, 행정 문서, 예산 정보 등
> - **서비스별 데이터 목록**: 실제 DB 컬럼 단위로 등급 분류 기재 권장
>   - CSAP D-13 심사 시 이 문서를 근거로 AI API 통제 여부를 확인합니다
> - **PII 마스킹 필드 목록**: 실제 서비스의 개인정보 필드 추가
>
> 이 문서에 실제 서비스 데이터가 반영되지 않으면 N2SF N-05 심사 시 지적 사항이 됩니다.
