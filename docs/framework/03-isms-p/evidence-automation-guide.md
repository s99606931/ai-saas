  ...
  monthly-report:
    ...
    steps:
      ...
      - name: 리포트 커밋
        run: |
          MONTH=$(date -d "last month" +%Y-%m)
          cp report.md docs/framework/03-isms-p/evidence-reports/isms-p-monthly-$MONTH.md
          git add docs/framework/03-isms-p/evidence-reports/
...
# 2. 최근 12개월 월간 리포트 포함
cp docs/framework/03-isms-p/evidence-reports/isms-p-monthly-*.md "$OUTPUT_DIR/"
