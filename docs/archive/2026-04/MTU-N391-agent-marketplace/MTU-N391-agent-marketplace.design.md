# MTU-N391-agent-marketplace — Design
...
| 보안 | CSAP D-08,D-09,D-06 준수 | 감사 로그 append-only |
| 운영 | In-memory + 감사 로그 영속화 | 최소 의존성 |
| 추적성 | FR-N391.1~5 ↔ 테스트 ↔ CSAP 매핑 | Q-Gate G1~G7 통과 |

## Context Anchor
- **WHY**: Multi-Agent 마켓플레이스 기능 확보 (공공 SaaS 프레임워크 필수)
- **WHO**: 테넌트 관리자, 운영자, 감사관
- **RISK**: 다중 테넌트 격리 실패, 감사 로그 누락
- **SUCCESS**: 전 테스트 PASS + matchRate 100%
- **SCOPE**: lib/agent-marketplace.ts + vitest 테스트 (4~8건)
...
## 데이터 모델
`src/lib/agent-marketplace.ts` 참조 — FR-N391.1~5 대응 interface 정의
