...
### F.4 AppShellTemplate — 최상위 App Shell (v2 신규)

> §K.3의 전체 구현 참조. 모든 포털 페이지의 루트 레이아웃.

```typescript
// Design Ref: §F.4, §K.3
// Plan FR: FR-UP.22~27
// 파일: components/templates/AppShellTemplate.tsx
// 사용: app/layout.tsx 또는 각 포털의 root layout에서 래핑

// 전체 구현은 §K.3 참조
// Next.js app router 적용:
// app/(portal)/layout.tsx
//   └── <AppShellTemplate> ... </AppShellTemplate>

// 중요: 서버 컴포넌트(RSC)에서 직접 사용 금지 — 클라이언트 상태(useState) 사용
// 'use client' 필수
```
...
## I. AiSidePanel — AI 우측 슬라이드 패널 (FR-UP.24)

> **v2 신규**: AI 패널이 우측에서 슬라이드 인, 본문을 왼쪽으로 밀기  
> 드래그 핸들로 패널 폭 260~600px 실시간 조절 가능

```typescript
// Design Ref: §I
// Plan FR: FR-UP.24
// 파일: components/organisms/AiSidePanel.tsx

interface AiSidePanelProps {
  isOpen: boolean
  width: number                    // 현재 폭 (260~600px)
...
// CSS 레이아웃 규칙:
// .ai-panel { flex-shrink:0; display:none; width:var(--ai-panel-w); }
// .ai-panel.open { display:flex; flex-direction:column; }
// → app-body flex row에서 flex-shrink:0이므로 main-area를 왼쪽으로 밀어냄

const AI_PANEL_MIN = 260
const AI_PANEL_MAX = 600
const AI_PANEL_DEFAULT = 360

export function AiSidePanel({ isOpen, width, onWidthChange, onClose, messages, onSendMessage }: AiSidePanelProps) {
  const panelRef = useRef<HTMLDivElement>(null)
  const resizeState = useRef({ active: false, startX: 0, startW: 0 })

  // 드래그 리사이즈 핸들러
  const handleResizeStart = useCallback((e: React.MouseEvent) => {
    if (window.innerWidth <= 768) return  // 모바일에서 리사이즈 비활성
    resizeState.current = { active: true, startX: e.clientX, startW: width }
    e.preventDefault()
  }, [width])
  ...
  useEffect(() => {
    const onMove = (e: MouseEvent) => {
      ...
      const diff = resizeState.current.startX - e.clientX
      const newW = Math.max(AI_PANEL_MIN, Math.min(AI_PANEL_MAX, resizeState.current.startW + diff))
      onWidthChange(newW)
    }
  ...
  return (
    <aside
      ref={panelRef}
      className="ai-panel open flex flex-col border-l border-[--color-border] bg-[--color-surface] shrink-0"
      style={{ width: `${width}px` }}
      aria-label="AI 어시스턴트"
    >
      {/* 드래그 리사이즈 핸들 (패널 왼쪽 가장자리) */}
      <div
        className="ai-resize-handle absolute left-0 top-0 bottom-0 w-1 cursor-col-resize hover:bg-[--color-primary] hidden md:block"
      ...
      <div className="flex items-center justify-between px-4 py-3 border-b border-[--color-border]">
        <div className="flex items-center gap-2">
          <Sparkles className="size-4 text-[--color-primary]" />
          <span className="font-semibold text-sm">AI 어시스턴트</span>
        </div>
        <button onClick={onClose} className="p-1 rounded hover:bg-[--color-hover]" aria-label="AI 패널 닫기">
          <X className="size-4" />
        </button>
      </div>
      ...
      <div className="p-3 border-t border-[--color-border]">
        <AiInputBar onSend={onSendMessage} />
      </div>
    </aside>
  )
}
...
**모바일 동작 (≤768px)**:
- `position:fixed; inset:0; bottom:56px (하단탭 위)` → 전체 화면 오버레이
- 드래그 리사이즈 핸들 숨김 (`hidden md:block`)
- 스와이프 다운으로 닫기 (터치 이벤트)

---

## J. RecentHistorySystem — 최근 방문 히스토리 (FR-UP.25, FR-UP.26)

> **v2 신규**: 최근 방문한 메뉴/서비스를 브라우저 탭처럼 상단에 표시  
> 탭(tabs) vs 목록(list) 모드를 개인 설정으로 선택 가능

### J.1 RecentTabsBar — 상단 탭 히스토리

```typescript
// Design Ref: §J.1
// Plan FR: FR-UP.25
// 파일: components/organisms/RecentTabsBar.tsx
// 위치: main-area의 첫 번째 flex 자식 (레일 바로 오른쪽부터 시작)
...
interface RecentTabsBarProps {
  tabs: RecentTab[]
  activeTabId?: string
  onTabClick: (tab: RecentTab) => void
  onTabClose: (tabId: string) => void
}

// CSS: .recent-tabs-bar { height:var(--tabs-h); overflow-x:auto; display:flex; }
// main-area 첫 번째 자식이므로 레일 너비 이후 영역을 가득 채움

export function RecentTabsBar({ tabs, activeTabId, onTabClick, onTabClose }: RecentTabsBarProps) {
  return (
    <div
      className="recent-tabs-bar flex items-center border-b border-[--color-border] bg-[--color-surface-raised] overflow-x-auto shrink-0"
    ...
    >
      {tabs.map(tab => (
        <div
          key={tab.id}
          className={cn(
            "flex items-center gap-1.5 px-3 h-full border-r border-[--color-border] shrink-0 cursor-pointer",
            "text-xs whitespace-nowrap select-none",
            activeTabId === tab.id
              ? "bg-[--color-surface] text-[--color-text-primary] border-b-2 border-b-[--color-primary]"
        ...
        >
          {tab.serviceIcon && <tab.serviceIcon className="size-3 shrink-0" />}
          <span className="max-w-[120px] truncate">{tab.label}</span>
          <button
            onClick={e => { e.stopPropagation(); onTabClose(tab.id) }}
            className="ml-1 p-0.5 rounded hover:bg-[--color-hover] opacity-60 hover:opacity-100"
...
### J.2 RecentModePreference — 탭/목록 모드 사용자 선택 (FR-UP.26)

```typescript
// Design Ref: §J.2
// Plan FR: FR-UP.26
// 파일: hooks/useRecentMode.ts

export type RecentMode = 'tabs' | 'list'

export function useRecentMode() {
  const [mode, setMode] = useState<RecentMode>(() => {
    // localStorage에서 사용자 선호 복원
    return (localStorage.getItem('recentMode') as RecentMode) ?? 'tabs'
  })

  const updateMode = useCallback((newMode: RecentMode) => {
    setMode(newMode)
    localStorage.setItem('recentMode', newMode)
    // 감사 로그: 사용자 UI 선호 변경 기록 (N2SF O등급 — AI 전송 가능)
    auditLog({ actor: getCurrentUserId(), action: 'UI_PREF_CHANGE', target: `recentMode:${newMode}` })
  }, [])
...
| 모드 | `tabs` (기본) | `list` |
|------|------------|-------|
| 위치 | `RecentTabsBar` (main-area 상단) | FloatingSidebar 내 섹션 |
| 표시 방식 | 가로 스크롤 탭 (브라우저 탭 UX) | 플로팅 사이드바 최근 목록 |
| 최대 항목 | 8개 (초과 시 가로 스크롤) | 20개 (스크롤) |
| 설정 위치 | 상단 바 설정 아이콘 → 환경설정 | 동일 |

---

## K. MobileResponsiveArchitecture — 3단계 반응형 (FR-UP.27)

> **v2 신규**: 3개 CSS 미디어 쿼리 중단점 + `.mobile-sim` 클래스 기반 시뮬레이션

### K.1 중단점 전략

| 중단점 | 구분 | 레이아웃 변화 |
|-------|------|------------|
| `> 1024px` | 데스크톱 | 기본 (레일 + 사이드바 + AI 패널 모두 가능) |
| `769~1024px` | 태블릿 | AI 패널 폭 최소화, 사이드바 플로팅 강제 |
| `≤768px` | 모바일 | 레일 숨김, 탭바 숨김, 하단탭 표시, 사이드바/AI 고정 오버레이 |
| `≤480px` | 소형 모바일 | 폰트 축소, 패딩 축소, 최대 2컬럼 그리드 |

### K.2 모바일 핵심 CSS 규칙

```css
/* Design Ref: §K.2 — Plan FR-UP.27 */

/* 태블릿 (769~1024px): AI 패널 최소화 */
@media (max-width: 1024px) {
  .ai-panel { width: 280px !important; }
  .service-rail.expanded { width: var(--rail-w) !important; }  /* 확장 잠금 */
}

/* 모바일 (≤768px): 완전 재배치 */
@media (max-width: 768px) {
  .service-rail { display: none !important; }
  .recent-tabs-bar { display: none !important; }  /* 하단탭으로 대체 */
  ...
  .floating-sidebar.open { transform: translateY(0) !important; }
  ...
  .ai-resize-handle { display: none !important; }  /* 리사이즈 비활성 */
}
...
### K.3 AppShell 컴포넌트 (템플릿 최상위)

```typescript
// Design Ref: §K.3, §F.4
// Plan FR: FR-UP.1, FR-UP.22~27
// 파일: components/templates/AppShellTemplate.tsx

interface AppShellProps {
  children: React.ReactNode   // main-content 영역
  sidebarSections?: SidebarMenuSection[]
  serviceName?: string
}

export function AppShellTemplate({ children, sidebarSections, serviceName }: AppShellProps) {
  ...
  const { mode: recentMode } = useRecentMode()
  const { tabs, addTab, removeTab, activeTab } = useRecentTabs()

  return (
    <div className="app-shell flex flex-col h-screen overflow-hidden">
      <AppNavbar onAiToggle={() => setAiOpen(v => !v)} />
      ...
      <div className="app-body flex flex-1 overflow-hidden">
        ...
        {/* 메인 영역 */}
        <div className="main-area flex flex-col flex-1 overflow-hidden relative">
          {/* 최근 탭바 (tabs 모드 + 데스크톱에서만) */}
          {recentMode === 'tabs' && tabs.length > 0 && (
            <RecentTabsBar tabs={tabs} activeTabId={activeTab} onTabClick={navigateTo} onTabClose={removeTab} />
          )}
...
> `platform/packages/ui/src/` 기준 (v2.0 업데이트)
...
│   └── ...
│
├── organisms/                      # ★ MTU-U1-P (G-01 해결)
│   ├── ServiceRail.tsx             # FR-UP.1~3, FR-UP.22, §B.1 (v2: 확장형)
│   ├── FloatingSidebar.tsx         # FR-UP.3, FR-UP.23, §B.2 (v2: 플로팅/핀 모드)
...
│   ├── AiSidePanel.tsx             # ★ FR-UP.24, §I (v2 신규)
│   └── RecentTabsBar.tsx           # ★ FR-UP.25, §J.1 (v2 신규)
│
├── templates/                      # ★ MTU-U1-P (G-02 해결)
│   ├── AppShellTemplate.tsx        # ★ FR-UP.22~27, §K.3 (v2 신규 — 최상위 레이아웃)
...
│
├── hooks/                          # ★ v2 신규
│   ├── useRecentMode.ts            # FR-UP.26, §J.2 (탭/목록 사용자 선호)
│   └── useRecentTabs.ts            # FR-UP.25 (최근 탭 상태 관리)
│
├── styles/                         # ★ v2 신규
│   └── design-tokens.css           # --rail-w, --ai-panel-w 등 CSS 토큰
│
...
| FR-UP.13 | §B.2 | FloatingSidebar.tsx | sidebar.spec.tsx | - |
...
| **FR-UP.22** | **§B.1** | **ServiceRail.tsx** (expanded/collapsed) | rail-expand.spec.tsx | - |
| **FR-UP.23** | **§B.2** | **FloatingSidebar.tsx** (floating/pinned) | sidebar-pin.spec.tsx | - |
| **FR-UP.24** | **§I** | **AiSidePanel.tsx** (slide + drag resize) | ai-panel.spec.tsx | - |
| **FR-UP.25** | **§J.1** | **RecentTabsBar.tsx** | recent-tabs.spec.tsx | - |
| **FR-UP.26** | **§J.2** | **useRecentMode.ts** (localStorage) | recent-mode.spec.ts | - |
| **FR-UP.27** | **§K.2** | **design-tokens.css** + AppShellTemplate.tsx | responsive.spec.tsx | - |
...
| 2.0.0 | 2026-04-05 | 데모 검증 기반 대폭 개정: App Shell 레이아웃, 확장형 레일(FR-UP.22), 플로팅 사이드바 핀 모드(FR-UP.23), AI 슬라이드 패널+드래그 리사이즈(FR-UP.24), 최근 탭바(FR-UP.25), 탭/목록 사용자 선호(FR-UP.26), 3단계 반응형(FR-UP.27) 추가. 섹션 §I~§K 신규. 추적성 §L로 이동. | PM Agent (데모 docs/demo/platform-portal-demo.html 기반) |
