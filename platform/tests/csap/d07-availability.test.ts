...
describe('CSAP D-07: 가용성 — Graceful Shutdown', () => {
  for (const service of ALL_SERVICES) {
    const indexPath = resolve(PROJECT_ROOT, `platform/services/${service}/src/index.ts`);
...
describe('CSAP D-07: 가용성 — Readiness Probe (DB 연결 확인)', () => {
  ...
  it('api-gateway: /ready 엔드포인트 존재 (다운스트림 헬스 확인)', () => {
    const indexPath = resolve(PROJECT_ROOT, 'platform/services/api-gateway/src/index.ts');
    const content = readFileSync(indexPath, 'utf-8');
    expect(content).toContain("'/ready'");
...
describe('CSAP D-07: 가용성 — k8s 구성 검증', () => {
  ...
  it('readinessProbe가 /ready 엔드포인트를 사용', () => {
    ...
    for (const block of deploymentBlocks) {
      ...
      const readinessMatch = block.match(/readinessProbe:[\s\S]*?path:\s*(\S+)/);
  ...
  it('livenessProbe는 /health 엔드포인트를 사용 (경량 확인)', () => {
    ...
    for (const block of deploymentBlocks) {
      ...
      const livenessMatch = block.match(/livenessProbe:[\s\S]*?path:\s*(\S+)/);
