...
import { readServiceFile, serviceFileContains, ALL_SERVICES, AUTH_REQUIRED_SERVICES } from '../helpers/service-validator';
  ...
  describe('서비스 레지스트리 검증', () => {
    ...
    it('서비스 레지스트리에 모든 인증 필수 서비스가 등록되어 있어야 한다', () => {
      const serviceIds = [
        ...
        'subscriptions', 'billing', 'crm', 'ai', 'notifications',
        'files', 'audit', 'compliance', 'security',
      ];
    ...
    for (const svc of ALL_SERVICES) {
      it(`${svc}에 Dockerfile이 존재해야 한다`, () => {
        const result = serviceFileContains(
          `platform/services/${svc}/Dockerfile`,
          ['FROM', 'EXPOSE'],
        );
