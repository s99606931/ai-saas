...
describe('E2E: AI 데이터 등급 검증 (FR-N01.11, N2SF N-05)', () => {
  ...
  describe('데이터 등급 미들웨어 검증', () => {
    it('API 게이트웨이에 데이터 등급 검증 미들웨어가 존재해야 한다', () => {
      const result = serviceFileContains(
        ...
        ['dataGradeMiddleware', 'DATA_GRADE_VIOLATION'],
      );
  ...
  describe('AI 서비스 등급 검사 검증', () => {
    it('ai-service에 데이터 등급 검사 로직이 존재해야 한다', () => {
      const result = serviceFileContains(
        ...
        ['validateDataGrade', 'DataGradeViolationError'],
      );
      expect(result.exists).toBe(true);
    });
    ...
    it('C등급 데이터 차단 테스트가 존재해야 한다', () => {
      const result = serviceFileContains(
        'platform/services/ai-service/tests/unit/grade-check.test.ts',
        ['C등급', '차단'],
      );
      expect(result.exists).toBe(true);
    });
  ...
  describe('PII 마스킹 검증', () => {
    ...
    it('PII 마스킹 테스트가 존재해야 한다', () => {
      const result = serviceFileContains(
        'platform/services/ai-service/tests/unit/pii-masking.test.ts',
        ['마스킹', 'PII'],
      );
      expect(result.exists).toBe(true);
    });
