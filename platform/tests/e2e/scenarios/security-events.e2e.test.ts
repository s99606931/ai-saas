...
describe('E2E: 보안 이벤트 흐름 (FR-N01.9, CSAP D-06, D-08)', () => {
  ...
  describe('보안 모니터링 서비스 검증', () => {
    it('security-monitor-service가 존재해야 한다', () => {
      const result = serviceFileContains('platform/services/security-monitor-service/src/index.ts', [
  ...
  describe('알림 서비스 보안 이벤트 통합 검증', () => {
    it('notification-service에 보안 이벤트 처리 로직이 존재해야 한다', () => {
      const result = serviceFileContains('platform/services/notification-service/src/index.ts', ['notification']);
      expect(result.exists).toBe(true);
    });
  });
