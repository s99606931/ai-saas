...
describe('E2E: 감사 추적 (FR-N01.5, CSAP D-06)', () => {
  ...
  describe('감사 서비스 핵심 기능 검증', () => {
    it('audit-service에 로그 기록 핸들러가 존재해야 한다', () => {
      const result = serviceFileContains('platform/services/audit-service/src/handlers/audit.handler.ts', [
        'POST',
        'audit',
      ]);
    ...
    it('감사 로그가 append-only 구조여야 한다 (CSAP D-06)', () => {
      const result = serviceFileContains('platform/services/audit-service/src/handlers/audit.handler.ts', ['append']);
      // append-only 패턴이 핸들러 또는 별도 테스트에 구현됨
      const testResult = serviceFileContains('platform/services/audit-service/tests/unit/append-only.test.ts', [
        'append-only',
        'immutable',
      ]);
    ...
    it('감사 로그 보존 정책(retention)이 구현되어야 한다', () => {
      const result = serviceFileContains('platform/services/audit-service/src/handlers/retention.handler.ts', [
        'retention',
      ]);
      expect(result.exists).toBe(true);
    });
  ...
  describe('감사 로그 무결성 검증', () => {
    it('audit-sdk에 SHA-256 해시 체인 로직이 존재해야 한다', () => {
      const result = serviceFileContains('platform/packages/audit-sdk/src/audit-logger.ts', [
        'sha',
        'hash',
        'integrity',
      ]);
      // integrity 테스트 파일에서도 확인
      const testResult = serviceFileContains('platform/packages/audit-sdk/tests/integrity.test.ts', [
        'SHA-256',
        'integrity',
      ]);
      expect(result.exists || testResult.exists).toBe(true);
  ...
  describe('서비스별 감사 로그 통합 검증', () => {
    it('auth-service에서 감사 로그를 기록해야 한다', () => {
      const auditLib = readServiceFile('platform/services/auth-service/src/lib/audit.ts');
    ...
    it('ai-service에서 감사 로그를 기록해야 한다', () => {
      const result = serviceFileContains('platform/services/ai-service/src/lib/audit.ts', ['audit']);
      expect(result.exists).toBe(true);
    });
  });
