...
export async function GET(request: NextRequest): Promise<NextResponse<SubscriptionListResponse | { error: string }>> {
  ...
  try {
    ...
    return NextResponse.json({ error: '구독 목록 조회 중 오류가 발생했습니다.' }, { status: 500 });
  }
}
