...
export async function registerTenantRoutes(app: FastifyInstance): Promise<void> {
  ...
  app.get(
    '/tenants/search',
    {
      schema: {
        ...
        querystring: {
          type: 'object' as const,
          properties: { q: { type: 'string' as const }, status: { type: 'string' as const } },
        },
  ...
  app.get(
    ...
    {
      schema: {
        description: '테넌트 목록 조회',
        tags: ['tenants'],
        querystring: {
          type: 'object' as const,
          properties: { page: { type: 'integer' as const }, limit: { type: 'integer' as const } },
        },
        response: { 200: listResponse },
      },
      preHandler: readLimiter,
  ...
  app.put(
    '/tenants/:id',
    {
      schema: {
        ...
        body: {
          type: 'object' as const,
          properties: { name: { type: 'string' as const }, plan: { type: 'string' as const } },
        },
        response: { 200: tenantResponse },
      },
      preHandler: writeLimiter,
  ...
  app.put(
    ...
    {
      schema: {
        description: '테넌트 상태 변경',
        tags: ['tenants'],
        params: idParam,
        body: {
          type: 'object' as const,
          required: ['status'] as const,
          properties: { status: { type: 'string' as const, enum: ['ACTIVE', 'SUSPENDED', 'DELETED'] } },
        },
        response: { 200: tenantResponse },
      },
      preHandler: writeLimiter,
