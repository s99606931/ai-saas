#!/bin/bash
# Velero 백업/복원 성능 모니터링 검증 스크립트
# Design Ref: MTU-N237 §3.7
# Plan SC: FR-N237.9
set -euo pipefail
...
print_header() {
  echo "=============================================="
  echo " Velero 백업/복원 모니터링 검증 (MTU-N237)"
...
# ── 1. Recording Rules 검증 ──
echo "[1/4] Velero 성능 Recording Rules"
RULES="infra/monitoring/velero-performance-rules.yaml"
if [ -f "$RULES" ]; then
  ...
  for rule in "velero:backup:success_count5m" "velero:backup:failure_count5m" \
              "velero:backup:success_ratio" "velero:backup:duration:p50" \
              "velero:backup:duration:p95" "velero:backup:size_bytes" \
              "velero:restore:success_count5m" "velero:restore:duration:p50" \
              "velero:schedule:last_success_age_seconds" "velero:schedule:success_ratio" \
              "velero:cpu:usage_cores" "velero:memory:usage_bytes"; do
...
# ── 2. 알림 규칙 검증 ──
echo "[2/4] 알림 규칙"
ALERTS="infra/monitoring/velero-performance-alerts.yaml"
if [ -f "$ALERTS" ]; then
  ...
  for alert in "VeleroBackupFailed" "VeleroBackupPartialFailure" \
               "VeleroScheduleStale" "VeleroRestoreFailed" \
               "VeleroBackupDurationHigh"; do
...
DASHBOARD="infra/monitoring/dashboards/velero-backup-performance.json"
if [ -f "$DASHBOARD" ]; then
  ...
  for tag in "velero" "backup" "dr"; do
...
# ── 4. 문서 완전성 ──
echo "[4/4] 문서 완전성"
for doc in "docs/01-plan/mtus/MTU-N237.plan.md" "docs/02-design/mtus/MTU-N237.design.md"; do
  if [ -f "$doc" ]; then
