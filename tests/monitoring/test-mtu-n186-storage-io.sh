#!/bin/bash
# =============================================================================
# MTU-N186: 스토리지 I/O 모니터링 E2E 테스트
# Design Ref: MTU-N186 Design §DS-N186.8
# Plan SC: FR-N186.8
# CSAP: D-12-05 (테스트 검증)
# =============================================================================
set -uo pipefail
...
RULES_FILE="$PROJECT_ROOT/infra/monitoring/storage-io-rules.yaml"
DASHBOARD_FILE="$PROJECT_ROOT/infra/monitoring/dashboards/storage-io.json"
...
echo "=============================================="
echo "MTU-N186 스토리지 I/O 모니터링 테스트"
echo "=============================================="

# --- TC-N186.1: YAML 문법 유효성 ---
echo ""
echo "[TC-N186.1] YAML 문법 유효성 검사"
...
# --- TC-N186.2: Recording Rule 수 검증 ---
echo ""
echo "[TC-N186.2] Recording Rule 수 검증"
...
# --- TC-N186.3: Alert Rule 수 검증 ---
echo ""
echo "[TC-N186.3] Alert Rule 수 검증"
...
# --- TC-N186.4: Dashboard JSON 유효성 ---
echo ""
echo "[TC-N186.4] Dashboard JSON 유효성 검사"
...
if [ "$DASHBOARD_UID" = "storage-io-n186" ]; then
...
# --- TC-N186.5: CSAP 라벨 매핑 확인 ---
echo ""
echo "[TC-N186.5] CSAP 라벨 매핑 확인"
...
# --- TC-N186.6: Design Ref 주석 존재 ---
echo ""
echo "[TC-N186.6] Design Ref 주석 존재 확인"
...
for rule_name in "disk:read_iops:rate5m" "disk:write_iops:rate5m" "disk:read_latency_ms:rate5m" "disk:write_latency_ms:rate5m" "disk:read_throughput:rate5m" "disk:write_throughput:rate5m" "disk:io_saturation:ratio" "container:fs_read_bytes:rate5m"; do
...
for alert_name in "DiskIOLatencyHigh" "DiskIOLatencyCritical" "DiskIOSaturationHigh" "DiskIOPSHigh" "DiskThroughputAnomaly" "DiskIOQueueDepthHigh"; do
